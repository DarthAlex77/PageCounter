    ...
    public class Worker : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            void Windows()
            {
                ...
                List<UsbPrinter> printers = PageCounter.Windows.PrinterResolver.GetUsbPrintersData();
                if (printers.Count != 0)
                {
                    foreach (UsbPrinter usbPrinter in printers)
                    {
                        usbPrinter.Jobs = PageCounter.Windows.JobsParser.ParseJobsForPrinter(usbPrinter);
                        if (usbPrinter.Jobs.Count != 0)
                        {
                            DbSender.SendToDb(printers, config, logger);
                        }
                    }
                }
                else
