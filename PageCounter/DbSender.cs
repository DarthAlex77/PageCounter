        ...
        public static void SendToDb(List<UsbPrinter> printers, ApplicationSettings config, ILogger<Worker> logger)
        {
            try
            {
                using (ApplicationContext db = new ApplicationContext(config))
                {
                    ...
                    {
                        UsbPrinter p = db.Printers.Include(x => x.Jobs).SingleOrDefault(x => x.HardwareSerial.Equals(usbPrinter.HardwareSerial));
                        if (p == null)
                        {
                            db.Printers.Add(usbPrinter);
                            db.SaveChanges();
                            logger.LogInformation($"Printer {usbPrinter.HardwareSerial} added to DB with {usbPrinter.Jobs.Count} job(s) at {DateTimeOffset.Now}");
                        }
                        else
                        {
                            ...
                            if (!p.LastKnowIp.Equals(usbPrinter.LastKnowIp))
                            {
                                ...
                                db.SaveChanges();
                                logger.LogInformation($"IP of {usbPrinter.HardwareSerial} changed to {usbPrinter.LastKnowIp} at {DateTimeOffset.Now}");
                            }
                        }
                    }
            ...
            catch (SqlException sqlException)
            {
                ...
                {
                    logger.LogError(sqlError.ToString());
                }
            }
